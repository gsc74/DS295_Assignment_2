...
void dijkstra(struct Graph* graph, int source) {
	struct MinHeap* minHeap = createMinHeap(num_vertices);
    distance[source] = 0;
		...
		for (int v = 0; v < num_vertices; ++v)
    {
		...
		minHeap->array[source] = 
          newMinHeapNode(source, distance[source]);
    ...
    while (!isEmpty(minHeap)) {
        ...
        while (pCrawl != NULL)
        {
            ...
            if (isInMinHeap(minHeap, v) && distance[u] != INF && distance[u] + weight < distance[v]) {
...
void print_shortest_paths(int source) {
    char fpath[256];
    sprintf(fpath, "../output/output_%d.bin", source);
    ...
    for (int i = 1; i < num_vertices; ++i) {
        ...
        fwrite(&(distance[i]), sizeof(int), 1, file);
        fwrite(&(path_length), sizeof(int), 1, file);

        for (int j = path_length - 1; j >= 0; --j) {
            fwrite(&(path[j]), sizeof(int), 1, file);
        }
        free(path);
    }
    fclose(file);
...
int main() {
    int source_vertex;
    FILE* file = fopen("../input/test_graph.mtx", "r");
    if (file == NULL) {
    ...
    distance = (int*)malloc(num_vertices * sizeof(int));
    ...
    while (fscanf(file, "%d %d %d\n", &n1, &n2, &weight) == 3) {
        // Add edge n1 -> n2
				...
				addEdge(graph,n2,n1,weight);
    }

    fclose(file);

    printf("Enter the source vertex: ");
    if (scanf("%d", &source_vertex) != 1 || source_vertex < 0 || source_vertex >= num_vertices) {
        printf("Invalid source vertex!\n");
        return EXIT_FAILURE;
    }

    dijkstra(graph,source_vertex);
    print_shortest_paths(source_vertex);
